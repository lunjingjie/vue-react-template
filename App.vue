...
<script setup lang="ts">
  ...
  const storedValue = getSessionValue('shouldLogout');
  let shouldLogoutValue;
  try {
    shouldLogoutValue = JSON.parse(storedValue || 'false');
  } catch (error) {
    console.error('Error parsing "shouldLogout" from session storage:', error);
    shouldLogoutValue = false;
  }
