export default {
  ...
  solar2lunar: function (y, m, d) {
    ...
    let temp = 0;
    //修正ymd参数
    const y1 = objDate.getFullYear();
    const m1 = objDate.getMonth() + 1;
    const d1 = objDate.getDate();
    let offset =
    ...
    let isToday = false;
    if (
      isTodayObj.getFullYear() == y1 &&
      isTodayObj.getMonth() + 1 == m1 &&
      isTodayObj.getDate() == d1
    ) {
      isToday = true;
    }
    //星期几
    ...
    const solarDate = y1 + '-' + m1 + '-' + d1;
    const lunarDate = year + '-' + month + '-' + day;

    const festival = this.festival;
    const lfestival = this.lfestival;
    const festivalDate = m1 + '-' + d1;
  ...
  lunar2solar: function (y, m, d, isLeapMonth) {
    ...
    m = parseInt(m);
    d = parseInt(d);
    const isLeapMonths = !!isLeapMonth;
    //   const leapOffset = 0;
    const leapMonth = this.leapMonth(y);
    //   const leapDay = this.leapDays(y);
    if (isLeapMonths && leapMonth != m) {
      return -1;
